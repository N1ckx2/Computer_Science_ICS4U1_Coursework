...
public class PresidentCardGame
{
    ...
    { //empties the pile into the trash and resets the players who have passed
        ...
        gui.info("Cards were moved to the trash!"); //prints to the GUI Label
    ...
    public PresidentCardGame() //contrustor to initalize the variables
    {
        ...
        while (playAgain)
        {
            gui = new GUI(this, second);
            gui.setVisible(true);
            deck = new Deck(); //deck of 52
            ...
            {
                if (player[i].hand().get(0).value() == 0)
                {
                    lastPlayed = i;
                    setLead();
                }
            ...
            String endgame = "<html><p>Game Over! The top players are: </p> <ol>";// starts string that shows rankings
            ...
            for (int i = 0; i < player.length; i++)
            {
                ...
                {
                    endgame = endgame +  "<li> " + player[i] + "</li>"; //adds all the players to the rankings in order
                    checker++;
                    i = 0;
                }
            }
            endgame = endgame + "</ol></html>";
